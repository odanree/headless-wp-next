...
// ─── DO Seed Articles ─────────────────────────────────────────────────────────
//
// Seeds 5 demo member_article posts on the live DigitalOcean Droplet.
// Skips any article whose slug already exists — safe to run multiple times.
//
// Usage (on the Droplet, after do-setup.sh):
//   php /root/do-seed-articles.php
//
// do-setup.sh automatically copies this file to /root/ with the correct
// WP path substituted.  The placeholder below matches create-wp-articles.php
// so the same sed one-liner works:
//   sed "s|/var/www/html/wp-load.php|/var/www/${DOMAIN}/htdocs/wp-load.php|g"
...
foreach ($articles as $a) {
    ...
    $existing = get_page_by_path($a['slug'], OBJECT, 'member_article');
    if ($existing) {
        echo "SKIP (exists) : " . $a['title'] . PHP_EOL;
        $skipped++;
        continue;
    }
...
echo PHP_EOL;
echo "Done. Created: $created  Skipped (already existed): $skipped" . PHP_EOL;
