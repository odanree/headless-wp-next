...
export async function POST(request: Request) {
  ...
  // Accept either a single `tag` string or a `tags` array — both shapes are valid.
  // The WP plugin sends individual tags; batch callers can send an array.
  ...
  if (!body?.secret || tags.length === 0) {
    return NextResponse.json({ error: 'Missing secret or tag/tags' }, { status: 400 });
  }
